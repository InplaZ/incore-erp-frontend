...
export default function NuevoClienteModal({
  ...
  const [form, setForm] = useState<CuentaComercialCreate>({
    ...
    ejecutivo_asignado: client?.ejecutivo_asignado ?? null,
  });
  ...
  return (
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div>
                <div>
                  ...
                  <select
                    value={form.ejecutivo_asignado ?? ""}
                    onChange={(event) =>
                      setForm((current) => ({
                        ...current,
                        ejecutivo_asignado: event.target.value
                          ? Number(event.target.value)
                          : null,
                      }))
                    }
                    disabled={ejecutivosLoading}
                    className="
      w-full rounded-lg
    ...
    "
                  >
                    ...
                    {ejecutivos?.map((ejecutivo) => (
                      <option
                        ...
                        value={ejecutivo.id}
                      >
                        {ejecutivo.first_name || ejecutivo.last_name
                          ? `${ejecutivo.first_name} ${ejecutivo.last_name}`
                          : ejecutivo.username}
                      </option>
                    ))}
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    Usuario responsable de la gestión comercial del cliente.
                  </p>
                </div>
              </div>
              </div>
