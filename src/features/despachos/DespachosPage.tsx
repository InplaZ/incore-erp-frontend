              ...
              {filtered.map(d => (
                <Card key={d.id} hover>
                  <CardBody>
                    ...
                    <p className="text-sm text-foreground font-medium">{d.clienteNombre}</p>
                    <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1"><MapPin className="h-3 w-3" /> {d.direccion}</p>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">Pedido: <span className="font-medium text-foreground">{d.pedidoNumero}</span></p>
                  </CardBody>
                </Card>
              ))}
            </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button onClick={() => setCalMonth(new Date(calMonth.getFullYear(), calMonth.getMonth() - 1, 1))} className="p-1.5 rounded-lg hover:bg-muted transition"><ChevronLeft className="h-4 w-4" /></button>
                <button onClick={() => setCalMonth(new Date(calMonth.getFullYear(), calMonth.getMonth() + 1, 1))} className="p-1.5 rounded-lg hover:bg-muted transition"><ChevronRight className="h-4 w-4" /></button>
              </div>
            </div>
          </CardHeader>
          <CardBody>
            <div className="grid grid-cols-7 gap-1">
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <div key={day} className={cn('min-h-[80px] rounded-lg border p-1.5 text-xs', dayDespachos.length > 0 ? 'border-sidebar/20 bg-sidebar/10/30' : 'border-border')}>
                    <p className="text-muted-foreground font-medium mb-1">{day}</p>
                    {dayDespachos.map(d => (
